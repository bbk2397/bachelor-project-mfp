...
<?php
	...
	$create_textarea_placeholder_text = $enter_solution_text_placeholder_text;
...
<?php
	...
	while( $row = $select_stmt->fetch(PDO::FETCH_ASSOC) ) {
		...
		$solution_number_text = htmlentities($solution_number)."\r\n";
		$version_text = htmlentities($row['version'])."\r\n";
		$since_text = htmlentities($row['since']);
		$number_version_since_textarea = '<textarea data-toggle="tooltip" data-placement="top" title="'.$solution_number_version_since_tool_tip_text.'" data-html="true" type="text" cols="18" rows="3" readonly>'.$solution_number_text.$version_text.$since_text.'</textarea>';
		$text_textarea = '<textarea data-toggle="tooltip" data-placement="top" title="'.$solution_text_tool_tip_text.'" type="text" cols="30" rows="3" readonly>'.htmlentities($row['text']).'</textarea>';
?>
					<tr>
						<td><?=$row_number?></td>
						...
						<td>
							<form method="post" style="display: inline;">
								<textarea type="text" name="text" cols="30" rows="3" placeholder="<?=$solution_text_update_placeholder_text?>" required></textarea>
								<input type="hidden" name="id" value="<?=$solution_id?>">
								<input type="hidden" name="solution_number" value="<?=$solution_number?>">
